...
const info = (...params: unknown[]) =>
  ENV.NODE_ENV !== 'test' && params.forEach((p) => winstonLogger.info(paramToLoggerFormat(p)));

const error = (...params: unknown[]) =>
  ENV.NODE_ENV !== 'test' && params.forEach((p) => winstonLogger.error(paramToLoggerFormat(p)));

const warning = (...params: unknown[]) =>
  ENV.NODE_ENV !== 'test' && params.forEach((p) => winstonLogger.warn(paramToLoggerFormat(p)));
